...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $luangiai = "Luận giải năm xem ".$yearWatch."\r\n".$thaitue . "\r\n" . $locton. "\r\n" . $thienma. "\r\n" . $thienkhoc. "\r\n" . $thienhu. "\r\n" . $tangmon;
    // Chia nhỏ nội dung thành các phần tối đa 1500 ký tự
    $luantuhoa = xuLyKetQua($luanhoaky, $luanhoakhoa, $luanhoaquyen, $luanhoaloc, $string);
...
function xuLyKetQua($luanhoaky, $luanhoakhoa, $luanhoaquyen, $luanhoaloc, $string) {
    $ketqua = "";
    
    // Kiểm tra từng biến và ghép vào kết quả nếu không rỗng
    if ($luanhoaky !== "") {
        $ketqua .= $luanhoaky;
    }
    ...
    // Nếu tất cả các biến đều rỗng, trả về giá trị của $string
    if ($ketqua === "") {
        $ketqua = $string;
    }
...
function LuanGiaiTuHoa($laSoData, $tenSao){
    ...
    //     "\r\n Bạn giao du với người khác giới sẽ có lúc đột nhiên đứt đoạn, không liên lạc nhau, không qua lại với nhau; hoặc có một khoảng thời gian không giao du bạn khác giới." .
